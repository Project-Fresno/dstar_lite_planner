...
public:
  PathPlanner() : Node("path_planner") {
    ...
    map_subscription = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
        "grid_msg", 10, std::bind(&PathPlanner::map_callback, this, _1));
    goal_subscription =
        this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "goal_pose", 10, std::bind(&PathPlanner::goal_callback, this, _1));
    pose_subscription = this->create_subscription<nav_msgs::msg::Odometry>(
        "odom", 10, std::bind(&PathPlanner::pose_callback, this, _1));
    timer_ = this->create_wall_timer(
        30ms, std::bind(&PathPlanner::timer_callback, this));
  }
...
private:
  ...
  void timer_callback() {
    ...
    if (this->flag_0 == true) {
      if (this->flag_1 == true) {
        this->dstar.updateStart(this->start_x / this->resolution,
                                this->start_y / this->resolution);
        this->dstar.replan();
  ...
  void map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr map) {
    ...
    for (int i = 0; i < this->map_height; i++) {
      for (int j = 0; j < this->map_width; j++) {
        ...
        //             this->dstar.updateCell(j+this->start_col,
        //             i+this->start_row, -1);
        //     }17
        RCLCPP_INFO(this->get_logger(), "%i", this->map_data[count]);

        if (this->map_data[count] == -1 || this->map_data[count] == 0)
          ;
        else {
          // RCLCPP_INFO(this->get_logger(), "%i", this->map_data[count]);
          this->dstar.updateCell(j + this->start_col, i + this->start_row,
                                 this->map_data[count]);
          // sqrt(pow((this->goal_x - this->start_x) / this->resolution,
          //          2) +
          //      pow((this->goal_y - this->start_y) / this->resolution,
          //          2)));
        }
        count++;
      }
  ...
  void goal_callback(const geometry_msgs::msg::PoseStamped::SharedPtr goal) {
    ...
    this->dstar.updateGoal(this->goal_x / this->resolution,
                           this->goal_y / this->resolution);
  }
  ...
  void pose_callback(const nav_msgs::msg::Odometry::SharedPtr pose) {
    ...
    // double siny_cosp = 2 * (pose.pose.orientation.w * pose.pose.orientation.z
    // + pose.pose.orientation.x * pose.pose.orientation.y); double cosy_cosp =
    // 1 - 2 * (pose.pose.orientation.y* pose.pose.orientation.y+
    // pose.pose.orientation.z * pose.pose.orientation.z); this->yaw =
